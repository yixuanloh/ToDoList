...
{
    public abstract class ToDoListWebTestBase : AbpAspNetCoreIntegratedTestBase<Startup>
    {
        protected static readonly Lazy<string> ContentRootFolder;

        static ToDoListWebTestBase()
        {
        ...
        protected override IWebHostBuilder CreateWebHostBuilder()
        {
            return base
                ...
                .UseSetting(WebHostDefaults.ApplicationKey, typeof(ToDoListWebMvcModule).Assembly.FullName);
        }
        ...
        {
            using (var context = IocManager.Resolve<ToDoListDbContext>())
            {
                action(context);
                context.SaveChanges();
            }
        ...
        protected async Task UsingDbContextAsync(Func<ToDoListDbContext, Task> action)
        {
            using (var context = IocManager.Resolve<ToDoListDbContext>())
            {
        ...
        protected async Task<T> UsingDbContextAsync<T>(Func<ToDoListDbContext, Task<T>> func)
        {
            T result;
